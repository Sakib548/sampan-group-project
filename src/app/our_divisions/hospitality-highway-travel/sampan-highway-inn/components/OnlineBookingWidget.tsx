...
const stayTiers = [
  { id: "transit", name: "Quick Rest (2–4 Hrs)", price: 1800, desc: "Hourly refresh, bath & relaxation between highway drives." },
  { id: "daypass", name: "Day Pass (6–8 Hrs)", price: 3800, desc: "Comfort suite + in-room dining + priority parking." },
  { id: "overnight", name: "Overnight Stay", price: 6500, desc: "Full 24-hour luxury accommodation + buffet breakfast." },
  { id: "event", name: "Party / Corporate Event", price: 15000, desc: "Banquet space, audio-visual suite, curated highway buffet." },
];

const roomTypes = [
  { id: "deluxe", name: "Deluxe King Suite (Soundproofed)", multiplier: 1 },
  { id: "twin", name: "Twin Executive Room", multiplier: 1 },
  { id: "family", name: "Family Interconnected Suite (4-6 Guests)", multiplier: 1.5 },
  { id: "vvip", name: "VVIP Presidential Highway Suite", multiplier: 2 },
];

const vehicleOptions = [
  { id: "car", name: "Standard Car / SUV", perk: "Complimentary Reserved Parking" },
  { id: "ev", name: "Electric Vehicle (EV)", perk: "Priority 120kW Supercharger Bay" },
  { id: "coach", name: "Microbus / Tourist Coach", perk: "Dedicated Heavy Vehicle Bay" },
  { id: "bike", name: "Motorbike / Touring Cruiser", perk: "Covered Security Bay" },
];
...
export default function OnlineBookingWidget({
  ...
  const [stayType, setStayType] = useState("daypass");
  ...
  return (
    <div id="booking-widget" className="relative z-20 mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-16 -mt-10 sm:-mt-14 mb-16">
      <div className="rounded-none border border-neutral-200 bg-white p-6 sm:p-8 lg:p-10 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.08)] text-neutral-950 relative">
        <div className="absolute top-0 left-0 h-[3px] w-full bg-emerald-700" />
        
        {/* Header Title */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-neutral-200">
          <div>
            <div className="inline-flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-emerald-800 mb-1.5">
              <HiSparkles />
              <span>Instant Highway Stopover Reservation</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-neutral-950">
              Plan Your Stop at Sampan Highway Inn
            </h2>
          </div>
          
          <div className="flex items-center gap-3">
            <span className="text-xs text-neutral-500 hidden sm:inline">Need instant phone help?</span>
            <a
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {stayTiers.map((tier) => (
              ...
              >
                ...
                <p className={`text-xl font-bold mt-1 ${stayType === tier.id ? "text-emerald-400" : "text-emerald-700"}`}>
        ...
        <form onSubmit={handleBookingSubmit} className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* Room / Suite Type */}
          <div className="bg-[#F5F5F2] border border-neutral-300 rounded-none p-4 focus-within:border-emerald-700 transition-colors">
          ...
          <div className="bg-[#F5F5F2] border border-neutral-300 rounded-none p-4 focus-within:border-emerald-700 transition-colors">
            <label className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-wider text-neutral-600 mb-2">
              <FaUserFriends className="text-emerald-700" />
          ...
          <div className="bg-[#F5F5F2] border border-neutral-300 rounded-none p-4 focus-within:border-emerald-700 transition-colors">
            <label className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-wider text-neutral-600 mb-2">
              <FaCar className="text-emerald-700" />
              <span>Vehicle / EV Bay</span>
            </label>
            <select
              value={vehicle}
          ...
          {/* Guest Name & Phone */}
          <div className="sm:col-span-2 bg-[#F5F5F2] border border-neutral-300 rounded-none p-4 focus-within:border-emerald-700 transition-colors">
          ...
          <div className="sm:col-span-2 lg:col-span-4 mt-2 flex flex-col md:flex-row md:items-center md:justify-between gap-4 pt-4 border-t border-neutral-200">
            <div className="flex items-baseline gap-3">
              ...
              <span className="text-xs text-emerald-800 font-medium font-mono">({selectedTier.name} • {selectedVehicle.perk})</span>
            </div>

            <div className="flex items-center gap-3">
              <button
                type="submit"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-3 rounded-none bg-neutral-950 hover:bg-emerald-700 px-10 py-4 font-mono text-xs font-bold uppercase tracking-[0.2em] text-white transition-all duration-300 cursor-pointer shadow-sm"
              >
        ...
        {isSubmitted && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="relative w-full max-w-lg rounded-none border border-neutral-300 bg-white p-8 text-neutral-950 shadow-2xl">
              ...
              <div className="w-16 h-16 rounded-none bg-emerald-50 border border-emerald-300 flex items-center justify-center text-emerald-700 text-3xl mb-6 mx-auto">
                <FaCheckCircle />
              </div>
              ...
              <p className="text-sm text-neutral-600 text-center mt-2 leading-relaxed">
                Thank you <strong className="text-emerald-800">{name}</strong>! Your stopover for{" "}
              ...
              <div className="mt-6 rounded-none bg-[#F5F5F2] p-4 border border-neutral-200 space-y-2 text-xs text-neutral-700 font-mono">
                <div className="flex justify-between">
                  <span className="text-neutral-500">Selected Suite:</span>
                  <span className="font-semibold text-neutral-950">{selectedRoom.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-neutral-500">Vehicle Perk:</span>
                  <span className="font-semibold text-emerald-800">{selectedVehicle.perk}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-neutral-500">Estimated Rate:</span>
              ...
              <div className="mt-6 flex flex-col gap-3">
                <a
                  href={`https://wa.me/8801929918408?text=Hello%20Sampan%20Highway%20Inn,%20I%20would%20like%20to%20confirm%20my%20booking%20for%20${encodeURIComponent(name)}%20on%20${encodeURIComponent(arrivalDate)}%20(${encodeURIComponent(selectedTier.name)})`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full inline-flex items-center justify-center gap-2 rounded-none bg-emerald-700 hover:bg-emerald-800 py-3.5 font-mono text-xs font-bold uppercase tracking-wider text-white transition-colors"
                >
