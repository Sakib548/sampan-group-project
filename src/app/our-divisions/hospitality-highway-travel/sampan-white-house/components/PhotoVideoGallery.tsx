...
export default function PhotoVideoGallery() {
  ...
  const handlePrev = (e: React.MouseEvent) => {
    ...
    if (lightboxIndex !== null) {
      setLightboxIndex(
        lightboxIndex > 0 ? lightboxIndex - 1 : galleryItems.length - 1
      );
    }
  };
  ...
  const handleNext = (e: React.MouseEvent) => {
    ...
    if (lightboxIndex !== null) {
      setLightboxIndex(
        lightboxIndex < galleryItems.length - 1 ? lightboxIndex + 1 : 0
      );
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {galleryItems.map((item, index) => (
            ...
            >
              ...
              {/* Top Category Badge */}
              <div className="absolute top-4 left-4 z-10 bg-black/70 backdrop-blur-md px-3 py-1 rounded-none text-[10px] font-bold uppercase tracking-widest text-[#e8b84b]">
                {item.category}
              </div>
