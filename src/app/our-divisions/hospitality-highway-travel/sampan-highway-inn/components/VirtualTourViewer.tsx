    ...
    <section id="virtual-tour" className="bg-[#050806] py-24 sm:py-32 text-white border-t border-white/10 relative overflow-hidden">
      <div className="mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <div className="inline-flex items-center gap-2 rounded-none border border-emerald-500/30 bg-emerald-950/60 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.25em] text-emerald-400 mb-4">
              <FaCompass />
          ...
          {tourZones.map((zone, idx) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 px-5 py-3 rounded-none text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${activeZoneIndex === idx
        ...
        <div className="relative rounded-none overflow-hidden border border-white/15 bg-neutral-950 shadow-2xl aspect-[16/9] max-h-[680px] min-h-[400px]">
          <Image
          ...
          <div className="absolute top-6 left-6 z-20 bg-black/60 backdrop-blur-md px-5 py-3 rounded-none border border-white/15 max-w-sm">
          ...
          {currentZone.hotspots.map((spot, idx) => (
            ...
            >
              ...
              >
                <span className="absolute w-8 h-8 rounded-none bg-emerald-400/40 animate-ping" />
                <span className="relative w-6 h-6 rounded-none bg-emerald-400 text-[#070b09] flex items-center justify-center font-bold text-xs shadow-lg group-hover:scale-125 transition-transform">
                  +
                </span>
              </button>
              ...
              {/* Hotspot Info Popup */}
              {selectedHotspot === idx && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-10 w-64 rounded-none bg-[#0d1410]/95 border border-emerald-500/40 p-4 text-white shadow-2xl backdrop-blur-xl z-30 animate-in fade-in zoom-in-95">
          ...
          <div className="absolute bottom-6 right-6 z-20 flex items-center gap-3">
            <button
              ...
              className="w-12 h-12 rounded-none bg-black/60 hover:bg-black/90 text-white border border-white/20 flex items-center justify-center transition-colors cursor-pointer"
            >
              <FaChevronLeft />
            </button>

            <span className="bg-black/60 px-4 py-2.5 rounded-none text-xs font-mono font-bold text-neutral-300 border border-white/10 backdrop-blur-sm">
          ...
          {/* Bottom Left Hint */}
          <div className="absolute bottom-6 left-6 z-20 hidden sm:flex items-center gap-2 text-xs text-neutral-400 bg-black/50 backdrop-blur-sm px-4 py-2 rounded-none border border-white/10">
